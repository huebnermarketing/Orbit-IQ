<template>
  <div
    class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden"
  >
    ...
    <div class="max-w-md w-full space-y-8 relative z-10">
      ...
      <div class="text-center">
        <div
          class="mx-auto h-20 w-20 flex items-center justify-center rounded-full bg-gradient-to-br from-purple-500 to-blue-600 mb-6 relative group animate-pulse-glow"
        >
          <div
            class="absolute inset-0 rounded-full bg-gradient-to-br from-purple-500 to-blue-600 animate-pulse opacity-75"
          ></div>
          <div
            class="absolute inset-0 rounded-full bg-gradient-to-br from-purple-500 to-blue-600 blur-lg opacity-50 group-hover:opacity-75 transition-opacity duration-300"
          ></div>
          <svg
            class="h-10 w-10 text-white relative z-10 transform group-hover:scale-110 transition-transform duration-300"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <h1
          class="text-4xl font-bold bg-gradient-to-r from-white via-purple-200 to-blue-200 bg-clip-text text-transparent mb-2"
        >
          Orbit IQ
        </h1>
      </div>
      ...
      <!-- Login Card -->
      <div
        class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-8 relative overflow-hidden"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-blue-500/10 rounded-3xl"
        ></div>
        ...
        <div class="relative z-10">
          ...
          <form @submit.prevent="handleLogin" class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <div class="relative group">
                <div
                  class="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-blue-500/20 rounded-xl blur opacity-0 group-focus-within:opacity-100 transition-opacity duration-300"
                ></div>
            ...
            >
              <span v-if="loading" class="absolute left-0 inset-y-0 flex items-center pl-3">
                <div class="animate-spin rounded-full h-5 w-5 border-b-2 border-white inline-block"></div>
              </span>
              <svg
                v-if="!loading"
                class="w-5 h-5 mr-2"
          ...
          <div
            v-if="error"
            class="mt-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl backdrop-blur-sm"
          >
            <div class="flex">
              <svg
      ...
      <div class="text-center mt-8">
        <p class="text-sm text-gray-400">© {{ currentYear }} Orbit IQ. All rights reserved.</p>
      </div>
    </div>
