...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const loadAvailableUsers = async () => {
      try {
        const response = await $fetch('/api/users/active') as any[]
    ...
    const initializeForm = () => {
      if (props.isEdit && props.userGroup) {
        ...
        form.user_ids = props.userGroup.users ? props.userGroup.users.map((user: any) => user.id) : []
      } else {
        form.name = ''
        form.description = ''
        form.color = '#3B82F6'
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (props.isEdit && props.userGroup) {
          await $fetch(`/api/user-groups/${props.userGroup.id}`, {
            method: 'PUT',
            body: data
          })
        } else {
