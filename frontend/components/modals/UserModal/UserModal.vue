      ...
      <div class="inline-block align-bottom bg-surface rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <div class="bg-surface px-6 pt-6 pb-4">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-2xl font-bold text-text-primary">{{ isEdit ? 'Edit User' : 'Create New User' }}</h3>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <div class="flex justify-center mb-6">
              <AvatarUpload :avatar-url="form.avatar ? `/storage/${form.avatar}` : null" :name="form.name" :size="80" @upload="handleAvatarUpload" @error="handleAvatarError" />
            </div>
            <div>
            ...
            <div>
              ...
              <input v-model="form.email" type="email" :disabled="!canEditEmail" :class="['input', !canEditEmail ? 'bg-gray-100 cursor-not-allowed' : '']" placeholder="Enter email address" required />
              <p v-if="!canEditEmail" class="text-xs text-gray-500 mt-1">Only super admin can modify email addresses.</p>
            </div>
            <div v-if="!isEdit">
            ...
            <div>
              <label class="block text-sm font-medium text-text-primary mb-2">System Role <span v-if="isClientLinkedUser" class="text-xs text-gray-500 ml-2">(Fixed for client users)</span></label>
            ...
            <div v-if="isEdit">
              ...
              <div class="flex items-center space-x-3">
                <label class="flex items-center cursor-pointer">
                  ...
                  <div class="relative">
                    <div class="w-11 h-6 rounded-full transition-colors duration-200 ease-in-out" :class="form.is_active ? 'bg-primary-500' : 'bg-gray-300'"></div>
                    <div class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ease-in-out" :class="form.is_active ? 'transform translate-x-5' : 'transform translate-x-0'"></div>
                  </div>
                  <span class="ml-3 text-sm font-medium text-text-primary">{{ form.is_active ? 'Active' : 'Inactive' }}</span>
                </label>
              </div>
              <p class="text-xs text-text-secondary mt-1">{{ form.is_active ? 'User can log in and access the system' : 'User cannot log in and is disabled' }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-2">Organization Roles <span v-if="isClientLinkedUser" class="text-xs text-gray-500 ml-2">(Fixed for client users)</span></label>
              <div v-if="isClientLinkedUser" class="space-y-2 border border-gray-200 rounded-lg p-3 bg-gray-50">
                <div class="flex items-center space-x-2">
                  <input type="checkbox" :value="14" checked disabled class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
              ...
              <div v-else class="space-y-2 max-h-32 overflow-y-auto border border-gray-200 rounded-lg p-3">
                <label v-for="orgRole in availableOrganizationRoles" :key="orgRole.id" class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                  <input type="checkbox" :value="orgRole.id" v-model="form.organization_role_ids" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
            ...
            <div v-if="isAMSelected">
              ...
              <select v-model="form.assigned_pm_id" class="input">
                ...
                <option v-for="pm in pmUsers" :key="pm.id" :value="pm.id">{{ pm.name }} ({{ pm.email }})</option>
              </select>
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              <button type="submit" :disabled="loading" class="btn-primary">{{ loading ? 'Saving...' : (isEdit ? 'Update User' : 'Create User') }}</button>
            </div>
          </form>
        </div>
      </div>
...
<script setup lang="ts" src="./UserModal.ts"></script>