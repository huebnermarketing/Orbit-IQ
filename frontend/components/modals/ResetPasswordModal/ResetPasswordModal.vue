    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-surface rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
        <div class="bg-surface px-6 pt-6 pb-4">
          ...
          <div class="mb-6">
            <p class="text-text-secondary">Reset password for <strong>{{ user?.name }}</strong> ({{ user?.email }})</p>
          </div>
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-text-primary mb-2">New Password</label>
              <input v-model="form.password" type="password" class="input" placeholder="Enter new password" required />
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-2">Confirm New Password</label>
              <input v-model="form.password_confirmation" type="password" class="input" placeholder="Confirm new password" required />
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              <button type="submit" :disabled="loading" class="btn-primary">{{ loading ? 'Resetting...' : 'Reset Password' }}</button>
            </div>
          </form>
        </div>
      </div>
...
<script  lang="ts" src="./ResetPasswordModal.ts"></script>
