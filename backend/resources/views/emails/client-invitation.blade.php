...
<head>
    ...
    <title>Welcome to Orbit IQ - {{ $clientPerson->client->company_name }}</title>
...
<body>
    <div class="email-container">
        ...
        <!-- Main Content -->
        <div class="content">
            <h1 class="greeting">Welcome, {{ $clientPerson->name }}!</h1>
            
            <p class="message">
                You have been invited to join <strong>{{ $clientPerson->client->company_name }}</strong> on Orbit IQ. 
                Your account has been created and you can now set up your password to access the platform.
            </p>

            <div class="company-info">
                <h3><i class="fas fa-building"></i> Company Information</h3>
                <p><strong>Company:</strong> {{ $clientPerson->client->company_name }}</p>
                <p><strong>Email:</strong> {{ $clientPerson->client->email }}</p>
                @if($clientPerson->client->phone)
                    <p><strong>Phone:</strong> {{ $clientPerson->client->phone }}</p>
                @endif
                @if($clientPerson->client->website)
                    <p><strong>Website:</strong> <a href="{{ $clientPerson->client->website }}" style="color: #667eea;">{{ $clientPerson->client->website }}</a></p>
                @endif
            </div>
        ...
        <div class="footer">
            ...
            <p class="footer-text">© {{ date('Y') }} Orbit IQ. All rights reserved.</p>
